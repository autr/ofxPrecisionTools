...
private:
    ...
    void set() {
        ...
        if (inner.y != scrollY) {
            inner.y = scrollY;
            setScrollBar();
            ofNotifyEvent(scrolledY, scrollY);
        }
    }
    
public:
    
    ofEvent<ofRectangle &> innerResized;
    ofEvent<ofRectangle &> outerResized;
    ofEvent<float &> scrolledY;
    ofEvent<float &> drawBars;
    ...
    void update() {
        ...
        if (innerDiff || outerDiff) {
            ...
            if (innerDiff) {
                ...
                ofNotifyEvent(innerResized, inner);
            }
            if (outerDiff) {
                lastOuterHeight = outer.height;
                lastOuterWidth = outer.width;
                ofNotifyEvent(outerResized, outer);
            }
            setScrollBar();
        }
    ...
    void pressed( int x, int y ) {
        ...
        if (scrollBarOuter.inside(x, y)) {
            ...
            dragOrigin.set(x, y);
            dragOriginalY = scrollY;
            isDragged = true;
        }
    }
    void dragged( int x, int y ) {
        if (isDragged) {
            scrollY = ofMap( y, outer.y, outer.y + outer.height, outer.y, ( outer.height - inner.height ) + outer.y, true);
